...
import { submitCardRequest, getCardRequests, type CardRequestItem } from '@/api/modules/card'
...
async function submit() {
  ...
  loading.value = true
  try {
    await submitCardRequest({ childId: childId.value, type: 'REPLACE', reason: reason.value, contact: contact.value })
    alert('补卡申请已提交')
    await load()
  } catch (e: any) {
...
<template>
  <div class="page">
    <div class="weui-panel weui-panel_access">
      <div class="weui-panel__hd">补卡申请</div>
      <div class="weui-panel__bd">
        ...
        <div class="weui-cells">
          <div class="weui-cell">
            <div class="weui-cell__bd">
              <textarea class="weui-textarea" placeholder="请输入原因" rows="3" v-model="reason"></textarea>
            </div>
          </div>
        </div>
        ...
        <div class="weui-cells">
          ...
          <a v-for="i in list" :key="i.id" class="weui-cell weui-cell_access" href="javascript:;">
            <div class="weui-cell__bd">
              <p><strong>{{ i.type==='REPLACE'?'补卡':(i.type==='LOSS'?'挂失':'解挂') }}</strong> · {{ i.status==='PENDING'?'审批中':(i.status==='APPROVED'?'已通过':'已拒绝') }}</p>
